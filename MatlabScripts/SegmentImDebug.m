function [Ilabel ,Clabel,ColIm] = SegmentImDebug(data,debug, params,Ilabel)
...
frame_no = int2str(params.frame_no);
...
if debug  icy_imshow(ColIm,['Frame ',frame_no,' - 4 - Neutralise Points w/o label']);  end
    ...
    function DoInitialSeeding()
        ...
        for f=1:s(3),
            ...
            if debug icy_imshow(I2(:,:,1)); input('press <enter> to continue','s');  end
            ...
            if debug  icy_imshow(Ilab(:,:,1)); input('press <enter> to continue','s');  end
...
%         end
%         if debug  icy_imshow(Ilabel2); input('press <enter> to continue','s');  end
%         Ilabel(:,:,f)=Ilabel2;
%     end
