...
EpiTools Version Tracker v.0.1 build S001 (beta)
 ...
 # according to releasing information from BAMBOO
 ...
 #             if (exists($ARGV_HASH->{release}))
 #             {
 #                 $release = $ARGV_HASH->{release};
 #                 $date_release = strftime "%a %d %b %Y %H:%M:%S (UTC %z)", localtime;
 #             }
 ...
 $release = $inputfile->{release} + 0.01;
 $date_release = strftime "%a %d %b %Y %H:%M:%S (UTC %z)", localtime;
