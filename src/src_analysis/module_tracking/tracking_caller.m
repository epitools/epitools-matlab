function tracking_caller(varargin)
%TRACKING_CALLER Function caller
% ------------------------------------------------------------------------------
% PREAMBLE
%
% This function will call a segmentation function on the image data. This
...
for i = 1:size(pool_instances(2:end),2)
    if (pool_instances(i+1).ref.active)
        ...
        clients(strcmp({clients.uid},'Tracking')).addArgv('STR01','argv',var);
        server.receiveMessage(clients(strcmp({clients.uid},'Tracking')),...
