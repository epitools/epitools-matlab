...
%% Get user preferences on the module
pref = getappdata(getappdata(0,'hMainGui'),'settings_execution');
...
if ~SingleFrame
    slider = uicontrol( fig ...
        ,'style'    ,'slider'   ...
        ,'units'    ,'normalized' ...
        ,'position' ,[0.11 0.005 0.78 0.025] ...
        );
...
uiTrackLabel = uicontrol('style','text', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    ...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.003 0.87 0.25 0.10], ...
    'String', 'Tracks',...
    'Parent',uiStatisticPanel);
uiTrackControl1 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    ...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.40 0.87 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiTrackControl2 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.70 0.87 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiCellsLabel     = uicontrol('style','text', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'left',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.003 0.77 0.40 0.10], ...
    'String', 'Cells',...
    'Parent',uiStatisticPanel);
uiCellsControl1 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.40 0.77 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiCellsControl2 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.70 0.77 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiCheckedLabel   = uicontrol('style','text', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'left',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.003 0.67 0.40 0.10], ...
    'String', 'OK Checked',...
    'Parent',uiStatisticPanel);
uiCheckedControl1 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.40 0.67 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiCheckedControl2 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.70 0.67 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiOrphansLabel   = uicontrol('style','text', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'left',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.003 0.57 0.40 0.10], ...
    'String', 'Orphans',...
    'Parent',uiStatisticPanel);
uiOrphansControl1 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
    'HorizontalAlignment', 'right',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.40 0.57 0.30 0.10], ...
    'String', '',...
    'Parent',uiStatisticPanel);
uiOrphansControl2 = uicontrol('style','edit', ...
    'units'    ,'normalized', ...
    'FontName','Tahoma',...
    'FontUnits','points',...
    'FontSize',9, ...
...
uiTrackingDistance_lbl = uicontrol(fig, ...
    ...
    'HorizontalAlignment', 'left',...
    'FontSize',9, ...
    'position' ,[0.003 0.55 0.20 0.45], ...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'String', 'Max tracking distance (pixels)',...
...
uiSeedRadius_lbl = uicontrol(fig, ...
    ...
    'HorizontalAlignment', 'left',...
    'BackgroundColor',[0.6000    0.6000    0.6000],...
    'position' ,[0.003 0.001 0.20 0.45], ...
    'String', 'Find local minimum within (pixels)',...
    'Parent',uiParametersPanel);
...
th = uitoolbar(fig);
    ...
    function icon_mat = matlab_icon(icon_file_name)
        [X,~,alpha] = imread(fullfile(matlabroot,...
            'toolbox','matlab','icons',icon_file_name));
        
        icon_mat = double(X)/256/256;
        icon_mat(~alpha) = NaN; 
        
        
        % Convert image from indexed to truecolor
        % previously for .gif
        %icon_mat = ind2rgb(icon_img,icon_map);
    end


% Open button
pth1 = uipushtool(th,'CData',matlab_icon('file_open.png'),...
    'TooltipString','Open a tracking file',...
    'HandleVisibility','off');

% Save button
pth2 = uipushtool(th,'CData',matlab_icon('file_save.png'),...
...
% Delete seed areas toogle
tth3 = uitoggletool(th,'CData',gif2cdata('images/gif/delete.gif'),'Separator','on',...
    'TooltipString','Delete seeded areas',...
=======
% Delete toogle 'CData',matlab_icon('tool_data_cursor.png')
tth3 = uitoggletool(th,'Separator','on',...
...
<<<<<<< HEAD
% Delete tracks toogle
tth11 = uitoggletool(th,'CData',gif2cdata('images/gif/chart_line_delete.gif'),'Separator','on',...
    'TooltipString','Delete tracks (using user preferences for determine the behavior)',...
    'HandleVisibility','off',...
    'Separator', 'on',...
    'ClickedCallback', {@keyPrsFcn,'t'});

% Add seed with automatic center toogle
tth9 = uitoggletool(th,'CData',gif2cdata('images/gif/target.gif'),'Separator','on',...
    'TooltipString','Autocentering seed',...
    'HandleVisibility','off',...
    'Separator', 'on',...
    'ClickedCallback', {@keyPrsFcn,'c'});
tth12 = uitoggletool(th,'CData',gif2cdata('images/gif/lock.gif'),'Separator','on',...
    'TooltipString','Retain function on click',...
    'HandleVisibility','off',...
    'Separator', 'on',...
    'ClickedCallback', {@keyPrsFcn,'0'});
=======
% Add seed with automatic center toogle 'CData',matlab_icon('help_ex.png'),
tth9 = uitoggletool(th,'Separator','on',...
    'TooltipString','Autocentering seed',...
    'HandleVisibility','off',...
    'Separator', 'on',...
    'ClickedCallback', {@keyPrsFcn,'c'},...
...
tth5 = uitoggletool(th,'CData',matlab_icon('tool_data_brush.png'),'Separator','on',...
...
% % Show tracks toogle 
>>>>>>> master
% tth10 = uitoggletool(th,'CData',gif2cdata('images/gif/chart_line.gif'),'Separator','on',...
%     'TooltipString','Show/Hide cell boundaries',...
%     'HandleVisibility','off',...
%     'ClickedCallback', {@keyPrsFcn,'h'});
% Grid toogle
tth8 = uitoggletool(th,'CData',matlab_icon('help_block.png'),'Separator','on',...
    'TooltipString','Show/Hide grid on image',...
    'HandleVisibility','off',...
    'ClickedCallback', {@keyPrsFcn,'g'});

% Legend toogle
tth10 = uitoggletool(th,'CData',matlab_icon('tool_legend.png'),'Separator','on',...
...
% Inspect toogle
tth6 = uitoggletool(th,'CData',matlab_icon('tool_font_italic.png'),'Separator','on',...
    'TooltipString','INSPECT trajectory problem',...
    'HandleVisibility','off',...
    'ClickedCallback', {@keyPrsFcn,'i'});

% Final inspect toogle
tth7 = uitoggletool(th,'CData',matlab_icon('tool_text_textbox.png'),'Separator','on',...
    'TooltipString','TEST incomplete trajectories',...
    'HandleVisibility','off',...
...
% Wipe toogle 
tth4 = uipushtool(th,'CData',matlab_icon('tool_font.png'),'Separator','on',...
    'TooltipString','Wipe ALL orphan seeds',...
    'HandleVisibility','off',...
    'ClickedCallback', {@keyPrsFcn,'b'},...
    'Visible','on');
...
% Evaluate the final check
    ...
    function img = Update()
        ...
        title(axes_subimg_b1,'Backward -1 frame', 'FontName', 'Tahoma', 'FontSize',9);
        title(axes_subimg_b2,'Backward -2 frames','FontName', 'Tahoma', 'FontSize',9);
        title(axes_subimg_f1,'Forward +1 frame', 'FontName', 'Tahoma', 'FontSize',9);
        title(axes_subimg_f2,'Forward +2 frames', 'FontName', 'Tahoma', 'FontSize',9);
        ...
        if(RemoveTrack);set(tth11,'State','on');else set(tth11,'State','off');end
        ...
        if(retainFunction);set(tth12,'State','on');else set(tth12,'State','off');end
        drawnow;
        bf_visual = [];
    ...
    function trajectories_statistics(idxtime,legend)
        ...
        % Set statistics data
        if isempty(oktrajs); oktrajs2plot = NaN; else oktrajs2plot = length(oktrajs); end
        set(uiTrackControl1,'string',tracksnum_curframe);
        set(uiTrackControl2,'string',tracksnum_total);
        set(uiCellsControl1,'string',cellnum_curframe);
        set(uiCellsControl2,'string',cellnum_total);
        %set(uiCheckedControl1,'string',NaN);
        set(uiCheckedControl2,'string',oktrajs2plot);
        set(uiOrphansControl1,'string',sum(orphan_seeds));
        %set(uiOrphansControl2,'string',NaN);
    end
                ...
                if ~delabelMode
                    ...
                    for n =1:length(cpy)
                        ...
                        if (checkSeedMatch(x,y,pt))
                            ...
                            % REMOVE
                            if ~RemoveTrack
                                %pref.analysis_modules.Tracking.ctl_deleteseedtrack.values{logical(pref.analysis_modules.Tracking.ctl_deleteseedtrack.actived)}
                                if ~InspectPt && ~okeydown; deletePt(x,y,CurrentFrame); end
                                
                                % ADD-PARTICLE
                            else
                                if(find(pref.analysis_modules.Tracking.ctl_deleteseedtrack.actived)==1)
                                    % Loop on the entire movie in order to
                                    % delete the whole track and the
                                    % correspondent seeds
                                    N = Itracks(y,x,CurrentFrame);
                                    for indexFrame = 1:NFrames;
                                        [a,b] = find(Itracks(:,:,indexFrame) == N);
                                        if ~isempty(a) && ~isempty(b)
                                            Itracks(Itracks(:,:,indexFrame) == N) = 0;
                                            Ilabel(Itracks==N) = 253;
                                            %neutralisePt(a,b,indexFrame);
                                            deletePt(b,a,indexFrame);
                                        end
                                    end
                                else
                                    N = Itracks(y,x,CurrentFrame);
                                ...
                                if ~retainFunction;RemoveTrack = false;end;
                            end
                            OnASeed = true;
                            break;
                        end
...
>>>>>>> master
            ...
            case {'i'}
                InspectPt = true;
            case {'b'}
                log2dev('Wipe orphan seed globally found in the frames', 'DEBUG');
                wipe_orphan_seeds();
                Update();
            case {'c'}
                if(localizeminimumvalue)
                    localizeminimumvalue = false;
                    log2dev('Key C pressed: automatic seed centering on minimum value OFF', 'DEBUG');
                else
                    localizeminimumvalue = true;
                    log2dev('Key C pressed: automatic seed centering on minimum value ON', 'DEBUG');
                end
                if(localizeminimumvalue);set(tth9,'State','on');else set(tth9,'State','off');end
                %Update();
            case {'0'}
    ...
    function [y2,x2] = find_local_minimum(x,y,intRadius)
        ...
        if ~retainFunction;localizeminimumvalue = false;end
