function polygoncrop_caller(varargin)
%POLYGONCROP_CALLER Function caller
% ------------------------------------------------------------------------------
% PREAMBLE
%
% This function will call a polygoncrop function on the image data. This
...
for i = 1:size(pool_instances(2:end),2)
    if (pool_instances(i+1).ref.active)
        ...
        clients(strcmp({clients.uid},'Polygon_Masking')).addArgv('POLY01','argv',var);
        server.receiveMessage(clients(strcmp({clients.uid},'Polygon_Masking')),...
