function projection_caller(varargin)
%PROJECTION_CALLER Function caller
% ------------------------------------------------------------------------------
% PREAMBLE
%
% This function will call a projection function on the image data. This
...
for i = 1:size(pool_instances(2:end),2)
    if (pool_instances(i+1).ref.active)
        ...
        clients(strcmp({clients.uid},'PROJECTION')).addArgv('PROJ01','argv',var);
        server.receiveMessage(clients(strcmp({clients.uid},'PROJECTION')),...
