...
abortcontrol = uicontrol('Style', 'pushbutton',...
    ...
    function save_and_close(src,eventdata)
        settingsobj = xml_read('usersettings.xml');
        setappdata(getappdata(0,'hMainGui'), 'settings_execution', settingsobj);
        server_instances = getappdata(getappdata(0, 'hMainGui'), 'server_instances');
        server_instances(2).ref.setFlushQueueBound(settingsobj.framework.serverupboundprocesses.ctl_serverupboundprocesses.values);
        delete(hUSGui); 
    end
            ...
            if(settingsobj.(root).(leaf).(char(nodes(i))).visible == 1)
                ...
                switch settingsobj.(root).(leaf).(char(nodes(i))).desc
                    case 'single'
                        ...
                        % Header description
                        if isfield(settingsobj.(root).(leaf).(char(nodes(i))),'extended')
                            uicontrol('Style','text',...
                                'String', char(settingsobj.(root).(leaf).(char(nodes(i))).extended),...
                                'Units', 'normalized',...
                                'HorizontalAlignment', 'left',...
                                'Position',[0.015 0.95 0.97 0.04 ],...
                                'FontAngle', 'italic', ...
                                'Parent',h,...
                                'HandleVisibility','on');
                        end
                        ...
                        for o=1:numel(settingsobj.(root).(leaf).(char(nodes(i))).values)
                            uicontrol('Style','radiobutton',...
                                ...
                                'Units', 'normalized',...
                                'Position',[0.01 (0.95-(0.04*o*i)) 0.4 0.04 ],...
                                'Parent',h,...
                    ...
                    case 'multiple'
                        ...
                        for u=1:length(settingsobj.(root).(leaf).(char(nodes(i))).values)
                            ...
                            uicontrol('Style','checkbox',...
                                      ...
                                      'Units', 'normalized',...
                                      'Position',[0.01 (0.95-(0.04*u*i)) 0.4 0.04 ],...
                                      'Parent',h,...
